...
- `LocalTools.Config.include` declares `local` in its dispatcher arms, so every caller must reach it with a function on the stack.
- A `$( )` command substitution inherits the function context of the function that forked it, so sourcing the include inside `$( )` from within a function is correct. Sourcing it inside `$( )` at a file's own top level is not.
- The top-level case fails twice over: bash 3.2 rejects the `local`, and a rejected `local` never performs its assignment, so the lookup key is empty, nothing matches, and the include returns the caller's default in place of the configured value.
- Each `console-*-bashrc.rc` reads `MDLT_CONSOLE_HISTORY` from its own top level, so the read is wrapped in a function defined inside the substitution, called with the arguments on the call line.
